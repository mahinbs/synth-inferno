    ...
    <nav className="relative">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 glass-content border border-black/20 rounded-xl shadow-2xl shadow-slate-900/10 backdrop-blur-2xl bg-white/10">
        <div className="flex justify-between items-center h-20 lg:h-24 relative overflow-hidden group">
          {/* Advanced glassmorphism background layer */}
          <div className="absolute inset-0 bg-gradient-to-r from-white/20 via-white/10 to-white/20 backdrop-blur-3xl opacity-80 group-hover:opacity-100 transition-all duration-500 ease-out" />
          
          {/* Animated gradient border */}
          <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 opacity-0 group-hover:opacity-30 transition-all duration-700 ease-out animate-gradient" />
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            ...
            <div className="absolute top-3/4 right-1/3 w-1 h-1 bg-purple-400/40 rounded-full animate-float" style={{ animationDelay: '2s' }} />
            <div className="absolute top-1/2 right-1/4 w-1.5 h-1.5 bg-pink-400/25 rounded-full animate-float" style={{ animationDelay: '4s' }} />
          </div>

          {/* Logo section with enhanced spacing and effects */}
          <div className="flex items-center flex-shrink-0 z-10 relative">
            <div className="p-2 rounded-lg bg-white/20 backdrop-blur-sm border border-white/30 shadow-lg hover:bg-white/30 transition-all duration-300 hover:scale-105 hover:shadow-xl">
              <Logo />
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center justify-center flex-1 px-8 relative z-10">
            <div className="bg-white/15 backdrop-blur-md rounded-full px-6 py-2 border border-white/20 shadow-lg hover:bg-white/25 transition-all duration-300 hover:shadow-xl">
              <DesktopNavigation 
                menuItems={menuItems} 
                isActive={isActive} 
          ...
          <div className="flex items-center space-x-4 z-10 relative">
            ...
            <div className="p-1 rounded-lg bg-white/20 backdrop-blur-sm border border-white/30 shadow-lg hover:bg-white/30 transition-all duration-300 hover:scale-105">
              <MobileMenuButton isOpen={isMenuOpen} onToggle={onToggleMenu} />
            </div>
          </div>

          {/* Dynamic glow effect */}
          <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 opacity-0 group-hover:opacity-50 transition-all duration-1000 ease-out blur-xl" />
        </div>
      </div>
