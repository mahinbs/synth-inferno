...
const FAQSection = memo(() => {
  ...
  return (
    <section 
      id="faq" 
      className="py-20 bg-gradient-to-b from-accent/10 to-background relative overflow-hidden"
    >
      <div className="container mx-auto px-6 relative z-10">
            ...
            >
              <div className={`mb-4 ${isVisible ? `animate-fade-in-up animate-stagger-${index + 2}` : 'opacity-0'}`}>
                <CollapsibleTrigger asChild>
                  <button className="w-full bg-card rounded-2xl p-6 border border-border hover:border-brand-primary/30 transition-all duration-300 text-left group">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg md:text-xl font-semibold text-card-foreground group-hover:text-brand-primary transition-colors duration-300">
                        {faq.question}
                      </h3>
                      <div className="w-8 h-8 bg-brand-primary/10 rounded-full flex items-center justify-center ml-4 group-hover:bg-brand-primary/20 transition-colors duration-300">
                        {openFAQ === faq.id ? (
        ...
        <div className={`text-center mt-16 ${isVisible ? 'animate-fade-in-up animate-stagger-4' : 'opacity-0'}`}>
          <div className="bg-gradient-to-r from-brand-primary/5 to-brand-secondary/5 rounded-3xl p-8 max-w-2xl mx-auto">
            ...
            <button className="bg-brand-primary text-white px-8 py-3 rounded-xl font-semibold hover:bg-brand-primary/90 transition-colors duration-300 hover:scale-105 transform">
              Contact Us
            </button>
          </div>
        </div>
