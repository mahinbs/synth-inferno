...
export const getPortfolioData = async (): Promise<Service[]> => {
  ...
  try {
    ...
    console.log(
      "PortfolioDataService - Static services loaded:",
      combinedData.length
    );
    ...
    try {
      ...
      console.log(
        "PortfolioDataService - Admin projects retrieved:",
      ...
      if (adminProjects && adminProjects.length > 0) {
        ...
        const filteredAdminProjects = adminProjects.filter(
          (project) =>
            project.id !== "1fdcbf8a-8a59-4c64-8a39-3b38ab9671ef" &&
            project.id !== "5ba46296-94a4-442b-97ad-c174d5e42665" &&
            project.serviceId !== "blockchain-development" &&
            project.serviceId !== "game-development"
        );

        console.log(
          "PortfolioDataService - Filtered admin projects:",
          filteredAdminProjects
        );

        // Group admin projects by service
        const adminProjectsByService = filteredAdminProjects.reduce(
          (acc, project) => {
            ...
            const transformedProject = {
              ...
              challenge:
                project.challenge || "Challenge details coming soon...",
        ...
        const uniqueAdminServiceIds = [
          ...new Set(filteredAdminProjects.map((p) => p.serviceId)),
        ];
        ...
        uniqueAdminServiceIds.forEach((serviceId) => {
          if (!existingServiceIds.includes(serviceId)) {
            ...
            let serviceColor: "cyan" | "blue" | "pink" | "purple" | "green" =
              "blue";
        ...
        combinedData.forEach((service) => {
          if (adminProjectsByService[service.id]) {
            service.projects = [
              ...service.projects,
              ...adminProjectsByService[service.id],
            ];
          }
        });

        console.log(
          "PortfolioDataService - Combined data with admin projects ready"
        );
      }
    } catch (adminError) {
      console.warn(
        "PortfolioDataService - Admin data failed, using static data only:",
        adminError
      );
    ...
    console.log(
      "PortfolioDataService - Final data:",
      combinedData.map((s) => ({
        id: s.id,
        title: s.title,
    ...
    console.error(
      "PortfolioDataService - Critical error, returning static fallback:",
      error
    );
